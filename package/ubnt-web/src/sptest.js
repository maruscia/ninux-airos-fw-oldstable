...
function handleStatus(response, textStatus)
{
	if (response.state == 10)
	{
		if (response.flags == 0)
		{
			var tx_res = Math.round(response.tx*100)/100;
			var rx_res = Math.round(response.rx*100)/100;
			$('#tx_results').html('' + response.tx.toFixed(2) + ' Mbps');
			$('#rx_results').html('' + response.rx.toFixed(2) + ' Mbps');
			$('#total_results').html('' + (response.tx + response.rx).toFixed(2) + ' Mbps');
			$('#results').html('');
		}
		else
		{
